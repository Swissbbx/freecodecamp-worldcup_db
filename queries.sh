...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams AS w ON games.winner_id = w.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT name FROM games LEFT JOIN teams AS o ON games.opponent_id = o.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
